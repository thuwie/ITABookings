    ...
    addEventElements() {
        ...
        btnOpenPopUp.addEventListener('click', async () => {
            ...
            try {
                ...
                const res = await fetch(`/food-court/${id}/province`);
                const { data } = await res.json();
                destination.innerHTML = `${data.foodCourt}, ${data.province.name}`;
...
document.addEventListener('DOMContentLoaded', () => FoodCourtHandler.init());
